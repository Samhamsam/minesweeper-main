...
public class Controller extends Observable implements IController {
    ...
    public void revealCell(Cell cell) {
        ...
        if (getState() == State.GAME_LOST || getState() == State.GAME_WON) return;
    ...
    public void recursiveRevealCell(Cell cell) {
        ...
        if (isFirstRound() && cell.hasMine()) {
            ...
            this.grid = new Grid(this.grid.getNumberOfRows(), this.grid.getNumberOfColumns(), this.grid.getNumberOfMines());
        ...
        if (cell.hasMine()) {
            setElapsedTime();
            this.state = State.GAME_LOST; // notifyObservers handled by wrapping revealCell() method
            return;
        }
        ...
        if (allCellsAreRevealed()) {
            setElapsedTime();
            this.state = State.GAME_WON; // notifyObservers handled by wrapping revealCell() method
            return;
        }
    ...
    /**
     * If a game is currently running, notify observers. Else, start new game with
     * standard values.
     */
    @Override
    public void touch() {
        if (this.grid != null) notifyObservers();
        else startNewGame();
    }
